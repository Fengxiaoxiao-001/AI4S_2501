...
def extract_final_scores_hard(root_path):
    """
    专门处理困难文件夹结构，提取所有Excel文件的绝对加权最终评分

    Parameters:
    root_path (str): 根目录路径（包含"困难"文件夹）
    ...
    # 困难文件夹路径
    hard_path = os.path.join(root_path, "困难")
    if not os.path.exists(hard_path):
        print(f"错误：困难文件夹路径不存在: {hard_path}")
        return None

    # 第二层：多智能体、角色扮演、无处理
    for method in ["多智能体", "角色扮演", "无处理"]:
        method_path = os.path.join(hard_path, method)
        ...
        for pattern in ["*.xlsx", "*.xls"]:
            excel_files.extend(glob.glob(os.path.join(method_path, pattern)))
        ...
        for excel_file in excel_files:
            try:
                ...
                qtype = name_parts[0]  # 题型：AQ, FBQ, MCQ, PSQ
                theme = name_parts[1]  # 主题：Alge, Geom, FunAndCal, StaAndPro
                file_id = name_parts[2] if len(name_parts) > 2 else "未知"

                # 验证题型和主题的合法性
                valid_qtypes = ["AQ", "FBQ", "MCQ", "PSQ"]
                valid_themes = ["Alge", "Geom", "FunAndCal", "StaAndPro"]

                if qtype not in valid_qtypes:
                    print(f"警告：无效的题型: {qtype}，文件: {filename}")
                    continue

                if theme not in valid_themes:
                    print(f"警告：无效的主题: {theme}，文件: {filename}")
                    continue
...
def save_consolidated_data_hard(df, output_file="困难_模型评分汇总.xlsx"):
    """
    保存合并后的数据到Excel文件（专门为困难文件夹结构优化）
...
if __name__ == "__main__":
    ...
    if not os.path.exists(root_directory):
        ...
        # 提取数据
        print("开始提取困难文件夹数据...")
        consolidated_data = extract_final_scores_hard(root_directory)

        if consolidated_data is not None:
            # 保存结果
            output_filename = "掌握程度M/困难_模型评分汇总.xlsx"
            save_consolidated_data_hard(consolidated_data, output_filename)
            ...
            if not consolidated_data.empty:
                print("\n文件类型分布:")
                type_dist = consolidated_data.groupby(['题型', '主题']).size().reset_index(name='文件数量')
                print(type_dist)
